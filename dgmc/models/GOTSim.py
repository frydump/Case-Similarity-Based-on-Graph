    ...
    def __init__(self, kernel_size, stride, in_channels, out_channels, num_sim_matrices):
        ...
        self.ops = torch.nn.ModuleList([torch.nn.Sequential(torch.nn.ReflectionPad2d((ka, kb, ka, kb)),
                                                            torch.nn.Conv2d(in_channels, out_channels, kernel_size,
                                                                            stride=stride, bias=bias)) for i in
                                        range(self.num_sim_matrices)])
...
class MaxPool2dLayer(torch.nn.Module):
    def __init__(self, kernel_size, stride, num_sim_matrices):
        ...
        self.ops = torch.nn.ModuleList([torch.nn.Sequential(torch.nn.ReflectionPad2d((ka, kb, ka, kb)),
                                                            torch.nn.MaxPool2d(kernel_size=self.kernel_size,
                                                                               stride=self.stride)) for i in
                                        range(self.num_sim_matrices)])
...
class CNNLayerV1(torch.nn.Module):
    def __init__(self, kernel_size, stride, in_channels, out_channels, num_similarity_matrices):
        ...
        self.layers = torch.nn.ModuleList([torch.nn.Conv2d(in_channels=self.in_channels, out_channels=self.out_channels,
                                                           kernel_size=self.kernel_size, stride=stride) for i in
                                           range(num_similarity_matrices)])
...
class MaxPoolLayerV1(torch.nn.Module):
    def __init__(self, stride, pool_size, num_similarity_matrices):
        ...
        self.layers = torch.nn.ModuleList(
            [torch.nn.MaxPool2d(kernel_size=self.pool_size, stride=stride) for i in range(num_similarity_matrices)])
...
class MaxPoolLayer(torch.nn.Module):
    def __init__(self, stride, pool_size, num_sim_matrices):
        ...
        if self.pool_size % 2 == 0:
            ...
            self.padding = torch.nn.ZeroPad2d((padding_temp, padding_temp, padding_temp, padding_temp))
        self.layers = torch.nn.ModuleList(
            [torch.nn.MaxPool2d(kernel_size=self.pool_size, stride=stride) for i in range(num_sim_matrices)])
...
def greedy_ot(cost_matrix, return_matching=False):
    ...
    cost_node_list = np.array(
        [np.array([cost_node(i, j, C_cpu[i, j]) for j in range(num_pts)]) for i in range(num_pts)]);
...
class MatrixSquareRoot(Function):
    ...
    def forward(ctx, input):
        ...
        if not np.isfinite(sqrtm_cpu).all():
            ...
            sqrtm_cpu = scipy.linalg.sqrtm(m + offset)
...
class GeometricGraphSim(torch.nn.Module):
    ...
    def setup_layers(self):
        ...
        for i in range(self.num_conv_layers):
            self.conv_layers.append(
                CNNLayerV1(kernel_size=self.args.conv_kernel_size[i], stride=1, in_channels=in_channels,
                           out_channels=self.args.conv_out_channels[i], num_similarity_matrices=self.num_gcn_layers))
            self.pool_layers.append(
                MaxPoolLayerV1(pool_size=self.args.conv_pool_size[i], stride=self.args.conv_pool_size[i],
    ...
    def forward(self, data):
        ...
        similarity_matrices_list = [torch.mm(abstract_features_list_1[i], abstract_features_list_2[i].transpose(0, 1))
                                    for i in range(self.num_gcn_layers)]
        if n1 > n2:
        ...
        similarity_matrices_list = [
            F.interpolate(_.unsqueeze(0).unsqueeze(0), size=self.args.interpolate_size, align_corners=True,
                          mode=self.args.interpolate_mode).squeeze(0).squeeze(0) for _ in similarity_matrices_list];
    ...
    def forward(self, data, return_matching=False):
        ...
        if 'distance_type' in self.args:
            if self.args.distance_type == 'negative_dot':
                ...
                abstract_features_list_1 = [F.normalize(abstract_features_list_1[i], dim=1) for i in
                                            range(self.num_gcn_layers)]
                abstract_features_list_2 = [F.normalize(abstract_features_list_2[i], dim=1) for i in
                                            range(self.num_gcn_layers)]
                ...
                deletion_similarity_matrices_list = [
                    torch.diag(compute_pairwise_distances_torch(abstract_features_list_1[i], self.deletion_params[i],
                                                                self.args.distance_type))
                    + insertion_constant_matrix
                    for i in range(self.num_gcn_layers)
                ...
                insertion_similarity_matrices_list = [
                    torch.diag(compute_pairwise_distances_torch(abstract_features_list_2[i], self.insertion_params[i],
                                                                self.args.distance_type))
                    + deletion_constant_matrix
                    for i in range(self.num_gcn_layers)
...
class UnnormalizedGOTSimWithLinear(GOTSim):
    ...
    def forward(self, data, return_matching=False):
        ...
        if 'matching_type' in self.args:
            ...
            if self.args.histogram == True:
                if 'histogram_all' in self.args and self.args.histogram_all:
                    hist = [self.calculate_histogram(abstract_features_1, torch.t(abstract_features_2)) for
                            (abstract_features_1, abstract_features_2) in
                            zip(abstract_features_list_1, abstract_features_list_2)]
...
class FrechetSim(GOTSim):
    ...
    def forward(self, data, return_matching=False):
        ...
        matching_cost = [2 * compute_g_FID_loss_by_iterative(f1, f2, normalized=True) / (n1 + n2) for f1, f2 in
                         zip(abstract_features_list_1, abstract_features_list_2)]
        ...
        if return_matching:
            return score, score_logits, matching_cost;
        else:
            return score, score_logits
